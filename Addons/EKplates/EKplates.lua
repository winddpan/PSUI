...
local function CreateAuraIcon(parent)
	...
	button:SetSize(C.auraiconsize, math.floor(C.auraiconsize*0.66))

	button.icon = button:CreateTexture(nil, "ARTWORK", nil, 3)
    button.icon:SetTexCoord(.1,.9,.1+(1-icon_ratio),.9-(1-icon_ratio))

	button.bg = button:CreateTexture(nil, 'ARTWORK', nil, 0)
    button.bg:SetTexture('interface/buttons/white8x8')
    button.bg:SetVertexColor(0,0,0,1)
    button.bg:SetAllPoints(button)
		
	button.icon:SetPoint("TOPLEFT",button.bg,"TOPLEFT", bd, -bd)
	button.icon:SetPoint("BOTTOMRIGHT",button.bg,"BOTTOMRIGHT",-bd, bd)	
	...
	button.text:SetPoint("TOPLEFT", button, "TOPLEFT", -1, 2)
	button.text:SetTextColor(1, 1, 0)
	
	button.count = createnumber(button, "OVERLAY", G.aurafontsize, G.fontflag, "RIGHT")
...
local function UpdateBuffs(unitFrame)
	...
	for index = 1, 15 do
		if i <= C.auranum then
			...
			if bname and matchbuff then
				...
				if i ~= 1 then
					unitFrame.icons[i]:SetPoint("LEFT", unitFrame.icons[i-1], "RIGHT", 2, 0)
				end
				i = i + 1
			end
		end
	...
	if i > 1 then
		unitFrame.icons[1]:SetPoint("LEFT", unitFrame.icons, "CENTER", -((C.auraiconsize+2)*(unitFrame.iconnumber)-4)/2,0)
	end
...
local function OnNamePlateCreated(namePlate)
	...
	if C.numberstyle then -- 數字樣式
		...
		namePlate.UnitFrame.castBar.Text = createtext(namePlate.UnitFrame.castBar, "OVERLAY", G.fontsize-5, G.fontflag, "CENTER")
		namePlate.UnitFrame.castBar.Text:SetPoint("TOPLEFT", namePlate.UnitFrame.castBar, "BOTTOMLEFT", -5, -1)
		...
		namePlate.UnitFrame.icons = CreateFrame("Frame", nil, namePlate.UnitFrame)
		namePlate.UnitFrame.icons:SetPoint("BOTTOM", namePlate.UnitFrame.name, "TOP", 0, 3)
		namePlate.UnitFrame.icons:SetWidth(140)
		namePlate.UnitFrame.icons:SetHeight(C.auraiconsize)
		namePlate.UnitFrame.icons:SetFrameLevel(namePlate.UnitFrame:GetFrameLevel() + 2)
