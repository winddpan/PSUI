...
--]]---------------------------------------------------------------------------------------------------------------------------------------
filters['itemLevel'] = {}
filters['itemLevel'].menuText = '物品等级'
filters['itemLevel'].data = {0, 999}
filters['itemLevel'].filterMessage = '物品等级不在0-999'

f = createFilterFrame('物品等级', 54)
f.bodyText:SetText('最低物品等级:')
...
--]]---------------------------------------------------------------------------------------------------------------------------------------
filters['sellPrice'] = {}
filters['sellPrice'].menuText = '卖价'
filters['sellPrice'].data = {0, 999999}
filters['sellPrice'].filterMessage = '卖价不在范围.'

f = createFilterFrame('卖价', 54)
f.bodyText:SetText('最低卖价:')
f.bodyText:SetPoint('TOPLEFT', 8, -14)
f.bodyText2 = f:CreateFontString()
f.bodyText2:SetFontObject("GameFontNormalSmall")
f.bodyText2:SetPoint('TOPLEFT', 8, -36)
f.bodyText2:SetTextColor(1, 1, 1)
f.bodyText2:SetText("最高卖价: ")
f.bodyText2:SetJustifyH("LEFT")
...
--]]---------------------------------------------------------------------------------------------------------------------------------------
filters['itemName'] = {}
filters['itemName'].menuText = '物品名称'
filters['itemName'].data = {'?'}
filters['itemName'].filterMessage = '物品名称不包含在文本中.'

f = createFilterFrame('物品名称', 32)
f.bodyText:SetText('物品名字包含文本:')
f.bodyText:SetPoint('TOPLEFT', 8, -14)
...
--]]---------------------------------------------------------------------------------------------------------------------------------------
filters['itemQuality'] = {}
filters['itemQuality'].menuText = '物品品质'
filters['itemQuality'].data = {[1] = true, [2] = true, [3] = true, [4] = true}
filters['itemQuality'].filterMessage = '不选择物品品质.'

f = createFilterFrame('物品品质', 50)
f.checkButtons = {}
f.checkButtons[1] = CreateFrame('CheckButton', nil, f, 'EasyScrapCheckButtonTemplate')
f.checkButtons[1]:SetPoint('TOPLEFT', 8, -8)
...
--]]---------------------------------------------------------------------------------------------------------------------------------------
filters['bindType'] = {}
filters['bindType'].menuText = '绑定类型'
filters['bindType'].data = {[1] = true, [2] = true, [4] = true}
filters['bindType'].filterMessage = '物品绑定类型不是所选绑定类型之一.'

f = createFilterFrame('绑定类型', 70)
f.checkButtons = {}
f.checkButtons[1] = CreateFrame('CheckButton', nil, f, 'EasyScrapCheckButtonTemplate')
f.checkButtons[1]:SetPoint('TOPLEFT', 8, -8)
f.checkButtons[1].text:SetText('装备绑定')
f.checkButtons[2] = CreateFrame('CheckButton', nil, f, 'EasyScrapCheckButtonTemplate')
f.checkButtons[2]:SetPoint('TOPLEFT', 8, -28)
f.checkButtons[2].text:SetText('灵魂绑定')
f.checkButtons[3] = CreateFrame('CheckButton', nil, f, 'EasyScrapCheckButtonTemplate')
f.checkButtons[3]:SetPoint('TOPLEFT', 8, -48)
f.checkButtons[3].text:SetText('绑定问题')
...
--]]---------------------------------------------------------------------------------------------------------------------------------------
filters['armorType'] = {}
filters['armorType'].menuText = '护甲类型'
...
filters['armorType'].filterMessage = '物品不是选定的护甲类型之一.'

f = createFilterFrame('护甲类型', 72)
f.checkButtons = {}
f.checkButtons[1] = CreateFrame('CheckButton', nil, f, 'EasyScrapCheckButtonTemplate')
f.checkButtons[1]:SetPoint('TOPLEFT', 8, -8)
...
--]]---------------------------------------------------------------------------------------------------------------------------------------
filters['weaponType'] = {}
filters['weaponType'].menuText = '武器类型'
...
filters['weaponType'].filterMessage = '物品不是任何选择的武器类型.'

f = createFilterFrame('武器类型', 308)
f.checkButtons = {}
...
--]]---------------------------------------------------------------------------------------------------------------------------------------
filters['itemType'] = {}
filters['itemType'].menuText = '物品类型'
filters['itemType'].data = {[LE_ITEM_CLASS_ARMOR] = true, [LE_ITEM_CLASS_WEAPON] = true}
filters['itemType'].filterMessage = '物品不是所物品类型之一.'

f = createFilterFrame('物品类型', 30)
f.checkButtons = {}
f.checkButtons[1] = CreateFrame('CheckButton', nil, f, 'EasyScrapCheckButtonTemplate')
f.checkButtons[1]:SetPoint('TOPLEFT', 8, -8)
f.checkButtons[1].text:SetText('护甲')
f.checkButtons[2] = CreateFrame('CheckButton', nil, f, 'EasyScrapCheckButtonTemplate')
f.checkButtons[2]:SetPoint('TOPLEFT', 128, -8)
f.checkButtons[2].text:SetText('武器')
...
--]]---------------------------------------------------------------------------------------------------------------------------------------
filters['bonusStats'] = {}
filters['bonusStats'].menuText = '辅助属性'
...
f.checkButtons['42'].text:SetText('速度')
...
--]]---------------------------------------------------------------------------------------------------------------------------------------
filters['itemSlot'] = {}
filters['itemSlot'].menuText = '装备部位'
...
filters['itemSlot'].filterFunction = function(itemIndex, filterIndex)
    ...
    if filterData[item.itemEquipLoc] or (item.itemEquipLoc == 'INVTYPE_ROBE' and filterData['INVTYPE_CHEST']) then 
        ...
        if invTypes[item.itemEquipLoc] then
            filters['itemSlot'].filterMessage = '装备部位是 '..invTypes[item.itemEquipLoc]
        else
            filters['itemSlot'].filterMessage = '装备部位未知.'
        end
        return false
    end
end
...
filters['equipmentSet'].menuText = '换装'
filters['equipmentSet'].filterMessage = '物品用于一个或多个换装.'


f = createFilterFrame('换装', 32)
f.bodyText:SetText('物品没有用于任何一个换装.')
...
--]]---------------------------------------------------------------------------------------------------------------------------------------
filters['duplicates'] = {}
filters['duplicates'].menuText = '重复'
filters['duplicates'].filterMessage = '不是一件重复物品.'


f = createFilterFrame('重复', 32)
f.bodyText:SetText('只显示重复物品.')
...
filters['transmogKnown'].menuText = '幻化'
filters['transmogKnown'].filterMessage = '你在收藏中没有这件物品.'

f = createFilterFrame('Transmog', 32)
f.bodyText:SetText('物品外观在您的收藏.')
...
filters['transmogKnown'].filterFunction = function(itemIndex, filterIndex)
    ...
    if not z then
        if item.itemClassID == LE_ITEM_CLASS_WEAPON or (item.itemClassID == LE_ITEM_CLASS_ARMOR and (item.itemSubClassID == LE_ITEM_ARMOR_CLOTH or item.itemSubClassID == LE_ITEM_ARMOR_LEATHER or item.itemSubClassID == LE_ITEM_ARMOR_MAIL or item.itemSubClassID == LE_ITEM_ARMOR_PLATE or item.itemSubClassID == LE_ITEM_ARMOR_SHIELD)) then 
            ...
            filters['transmogKnown'].filterMessage = '无法确定外观是否已知.\n有些物品会发生这种情况，请手动检查.'
            return false
        else
            return true 
        end
...
filters['bags'].filterMessage = '物品不在任何一个背包.'
...
f.checkButtons[1].text:SetText('背包 #1')
f.checkButtons[2] = CreateFrame('CheckButton', nil, f, 'EasyScrapCheckButtonTemplate')
f.checkButtons[2]:SetPoint('TOPLEFT', 8, -28)
f.checkButtons[2].text:SetText('背包 #2')
f.checkButtons[3] = CreateFrame('CheckButton', nil, f, 'EasyScrapCheckButtonTemplate')
f.checkButtons[3]:SetPoint('TOPLEFT', 128, -28)
f.checkButtons[3].text:SetText('背包 #3')
f.checkButtons[4] = CreateFrame('CheckButton', nil, f, 'EasyScrapCheckButtonTemplate')
f.checkButtons[4]:SetPoint('TOPLEFT', 8, -48)
f.checkButtons[4].text:SetText('背包 #4')
function f:populateData(data)
