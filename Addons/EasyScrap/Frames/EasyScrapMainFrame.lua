...
mainFrame.searchBox.Instructions:SetText('搜索')
...
filterSelection.text:SetText('筛选:')
...
filterSelection.Button:SetScript('OnClick', function() 
    if DropDownList1:IsVisible() then
        ...
        if not EasyScrap.scrapInProgress then
            ...
            DEFAULT_CHAT_FRAME:AddMessage('Easy Scrap: 拆解物品时无法切换筛选.')
        end
    end
end)
UIDropDownMenu_SetText(filterSelection, "默认")
...
filtersButton:SetScript('OnClick', function()
    if not EasyScrap.scrapInProgress then
        ...
        DEFAULT_CHAT_FRAME:AddMessage('Easy Scrap: 拆解物品时无法切换菜单.')
    end
end)
