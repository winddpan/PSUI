...
local Gains = {
	{name = "袖剑连击", unit = "player", filter = "BUFF", valueIndex = 16},
	...
	{name = "偷袭", unit = "target", filter = "DEBUFF", value = 0.1, talent = "5/3"},
	{name = "肾击", unit = "target", filter = "DEBUFF", value = 0.1, talent = "5/3"},
}
...
local function OnEvent(event, unit) 
	...
	for i = 1, #Gains, 1 do
		local data = Gains[i]
		local auraValue = 0
		local name = select(1, AuraUtil.FindAuraByName(data.name, data.unit, data.filter))
		if name == data.name then
			...
			if contains then
				...
				if data.valueIndex then
					local text = select(data.valueIndex, AuraUtil.FindAuraByName(data.name, data.unit, data.filter))
					if text then
						value = tonumber(text)
						if value then
							value = value/100.0
	...
	if AuraUtil.FindAuraByName("大师刺客的决意", "player") then
