...
local function UpdateQuestsDistance()
    if (QuestMapFrame and QuestMapFrame:IsVisible()) 
	        or not QuestWatchSortDB.enabled 
    ...
    if nearest and nearest > 0 then
        ...
        if (GetTime() > protectionTime and questID ~= GetSuperTrackedQuestID()) then
            ...
            if currDist and currDist - nearest > nearest * 0.07 + 1000 then
                ...
                --PlaySound("UI_WorldQuest_Map_Select", "master")
                --PlaySound("KeyRingClose", "master")
...
local function EnableOrDisable()
    if QuestWatchSortDB.enabled then
...
frame:SetScript("OnEvent", function(self, event)
    if(event == "VARIABLES_LOADED") then
        QuestWatchSortDB = QuestWatchSortDB or { enabled = true }
...
checkbox:SetScript("OnClick", function(self, button)
    QuestWatchSortDB.enabled = self:GetChecked()
    EnableOrDisable()
end)
...
checkbox:SetScript("OnEnter", function(self)
    ...
    GameTooltip:AddLine(L["Sort Quests"])
	GameTooltip:AddLine(L["Sort quest watches by distance of objectives. This was done by SortQuestWatches() before 7.0 which is broken currently."], 1, 1, 1, true)
    GameTooltip:Show();
end)
