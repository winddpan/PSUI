  ...
  lib.gen_fontstring = function(f, name, size, outline)
	...
	if outline == "OUTLINEMONOCHROME" then outline = "NONE" end
    fs:SetFont(name, size, outline)
    fs:SetShadowColor(0,0,0,1)
	if outline == "NONE" then fs:SetShadowOffset(1,-1)	end
  ...
  lib.gen_backdrop = function(f)
	...
	f.Shadow:SetPoint("TOPLEFT",  1, -1)
	f.Shadow:SetPoint("BOTTOMRIGHT", -1, 1)
	f.Shadow:SetBackdrop(backdrop_tab)
...
------ [Building frames]
  ...
  lib.gen_hpbar = function(f)
    ...
    b:SetAllPoints(s)
	--b:SetAlpha(0.48)
	b:SetVertexColor(0.1,0.1,0.1,0.7)
	-- threat border
  ...
  lib.gen_hpstrings = function(f, unit)
    ...
    h:SetFrameLevel(15)
    local valsize = 8
    local name = lib.gen_fontstring(h, cfg.oUF.media.font, 12, "NONE")
    local hpval = lib.gen_fontstring(h, cfg.oUF.media.pixFont, valsize, "THINOUTLINE")
	local hppercent = lib.gen_fontstring(h, cfg.oUF.media.pixFont, 8, "THINOUTLINE")
	if f.mystyle == "player" then
	  name:Hide()
      hpval:SetPoint("LEFT", f.Health, "LEFT",5,0)
    elseif f.mystyle == "target" then
      name:SetPoint("RIGHT", f.Health, "RIGHT",-3,0)
      hpval:SetPoint("LEFT", f.Health, "LEFT",5,0)
      name:SetJustifyH("RIGHT")
      name:SetPoint("LEFT", hpval, "RIGHT", 5, 0)
	  hppercent:SetPoint("CENTER", f.Health, "CENTER", 0, 0)
	  f:Tag(hppercent, '[mono:hpperc]')
    ...
    if f.mystyle == "arenatarget" or f.mystyle == "partypet" or  f.mystyle == "tot" or  f.mystyle == "focus" or  f.mystyle == "pet" then
      f:Tag(name, '[mono:color][mono:shortname]')
      --f:Tag(hpval, '[mono:hpraid]')
    else
      f:Tag(name, '[mono:color][mono:longname]')
      --f:Tag(hpval, '[mono:hp]')
	  f:Tag(hpval, '[mono:hpnormal]')
	  --f:Tag(hppercent, '[mono:hpperc]')
    end
  end
  ...
  lib.gen_ppstrings = function(f, unit)
    ...
    h:SetFrameLevel(10)
    local fh = 8
    local pp = lib.gen_fontstring(h, cfg.oUF.media.pixFont, fh, "THINOUTLINE")
...
------ [Castbar, +mirror castbar]
  ...
  lib.gen_castbar = function(f)
    ...
    b:SetTexture(cfg.oUF.media.statusbar)
    b:SetAllPoints(s)
    b:SetVertexColor(cfg.oUF.castbar.color.normal[1]*0.2,cfg.oUF.castbar.color.normal[2]*0.2,cfg.oUF.castbar.color.normal[3]*0.2,0.6)
    --spark
    local sp = s:CreateTexture(nil, "OVERLAY")
    sp:SetBlendMode("ADD")
    sp:SetAlpha(0.5)
    ...
    --spell text
    local txt = lib.gen_fontstring(s, cfg.oUF.media.font, 11, "NONE")
    txt:SetPoint("LEFT", 2, 0)
    txt:SetJustifyH("LEFT")
    --time
    local t = lib.gen_fontstring(s, cfg.oUF.media.pixFont, 8, "THINOUTLINE")
...
--[[	if unitCaster ~= 'player' and unitCaster ~= 'vehicle' and not UnitIsFriend('player', unit) and icon.debuff then
  ...
  lib.PostCreateIcon = function(self, button)
	...
	button.remaining = lib.gen_fontstring(button.cd, cfg.oUF.media.pixFont, cfg.oUF.settings.auratimers.font_size, "THINOUTLINE")
	...
	button.border:SetPoint("TOPLEFT", button.overlay, "TOPLEFT", -1, 1)
    button.border:SetPoint("BOTTOMRIGHT", button.overlay, "BOTTOMRIGHT", 1, -1)
  ...
  lib.createBuffs = function(f)
	...
	elseif f.mystyle=="boss" then
	  ...
	  b["growth-x"] = "LEFT"
	  b.initialAnchor = "TOPRIGHT"
      b:SetPoint("TOPRIGHT", f, "TOPLEFT", -5, -2)
	  b.size = 26
      b.num = 4
      b:SetWidth((b.size+b.spacing)*4)
...
------ [Extra functionality]
  ...
  lib.gen_alt_powerbar = function(f)
	...
	apb.b:SetPoint("BOTTOMRIGHT", apb, "BOTTOMRIGHT", 4, -4)
    lib.gen_backdrop(apb.b)
	--apb.b:SetBackdrop(backdrop_tab)
	--apb.b:SetBackdropColor(0, 0, 0, 0)
	--apb.b:SetBackdropBorderColor(0,0,0,1)
	
	apb.v = lib.gen_fontstring(apb, cfg.oUF.media.pixFont, 8, "THINOUTLINE")
