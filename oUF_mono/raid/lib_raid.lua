  ...
  local PostUpdateHealth = function(s, unit)
    ...
    if(b) then
	  ...
	  elseif not cfg.oUF.settings.ReverseHPbars then
			bg:SetVertexColor(r*.3, g*.3, b*.3, 0.33)
			s:SetStatusBarColor(r, g, b, .8)
      else
  ...
  local PostUpdatePower = function(power, unit)
    ...
    if(b) then
      local bg = power.bg
	if UnitIsDeadOrGhost(unit) or not UnitIsConnected(unit) then
			bg:SetVertexColor(r/2, g/2, b/2, 0.7)
			power:SetStatusBarColor(0, 0, 0, .5)
	  elseif not cfg.oUF.settings.ReverseHPbars then
			bg:SetVertexColor(r*.2, g*.2, b*.2, 0.33)
